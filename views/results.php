...
{
  $TitleExists = property_exists($Poll, 'Title');
  $HideTitle = C('Plugins.DiscussionPolls.DisablePollTitle', false);
  echo '<div class="DP_ResultsForm">';
  
    if($TitleExists || $HideTitle) {
      $TitleS = Gdn::formatService()->renderHtml($Poll->Title,  \Vanilla\Formatting\Formats\HtmlFormat::FORMAT_KEY);
      if(trim($Poll->Title) !== '') {
        $TitleS .= '<hr />';
      }
    }
...
function RenderQuestion($Question)
{
  ...
  echo Wrap(Gdn::formatService()->renderHtml($Question->Title, \Vanilla\Formatting\Formats\TextFormat::FORMAT_KEY), 'span');
  echo Wrap(sprintf(Plural($Question->CountResponses, '%s vote', '%s votes'), $Question->CountResponses), 'span', ['class' => 'Number DP_VoteCount']);
  ...
  foreach ($Question->Options as $Option) {
    ...
    echo Wrap($String, 'li', ['class' => 'DP_ResultOption']);
    $k = ++$k % 10;
  }
  echo '</ol>';
  echo '</li>';
