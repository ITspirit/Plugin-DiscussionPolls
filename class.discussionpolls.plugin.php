...
class DiscussionPollsPlugin extends Gdn_Plugin
{
    ...
    public function SettingsController_DiscussionPolls_Create($Sender)
    {
        ...
        $ConfigurationModel->SetField(['Plugins.DiscussionPolls.EnableShowResults']);
        $ConfigurationModel->SetField(['Plugins.DiscussionPolls.DisablePollTitle']);
    ...
    public function Controller_Submit($Sender)
    {
        ...
        if (!$Session->CheckPermission('Plugins.DiscussionPolls.Vote', false) || !$Session->UserID) {
            Gdn::Session()
                ->Stash('DiscussionPollsMessage', T('Plugins.DiscussionPolls.UnableToSubmit', 'You do not have permission to submit a poll.'));
            redirectTo('discussion/' . $FormPostValues['DiscussionID']);
        }
        ...
        if ($Sender->DeliveryType() == DELIVERY_TYPE_VIEW) {
            ...
            if ($Saved) {
                ...
                if ($Partial) {
                    Gdn::Session()
                        ->Stash('DiscussionPollsMessage', T('Plugins.DiscussionPolls.UnsweredAllQuestions', 'You have not answered all questions!'));
                } else {
    ...
    public function PostController_Render_Before($Sender)
    {
        ...
        if (!$Session->CheckPermission(['Plugins.DiscussionPolls.Add', 'Plugins.DiscussionPolls.Manage'], false)) {
    ...
    public function PostController_DiscussionFormOptions_Handler($Sender)
    {
        ...
        // render check box
        $Sender->EventArguments['Options'] .= '<li>' . $Sender->Form->CheckBox('DP_Attach', T('Attach Poll'), ['value' => '1', 'checked' => true])
            . '</li>';
        ...
        if (!empty($DiscussionPoll->PollID)) {
            ...
            echo Wrap(T('Plugins.DiscussionPolls.PollClosedToEdits',
                'You cannot edit a poll. You <em>may</em> delete this poll by unchecking the Attach Poll checkbox.'), 'div',
    ...
    public function DiscussionModel_BeforeSaveDiscussion_Handler($Sender)
    {
        ...
        $Invalid = false;
        $Error = '';
        if (!C('Plugins.DiscussionPolls.DisablePollTitle', false) && trim($FormPostValues['DP_Title']) === '') {
            $Invalid = true;
    ...
    public function DiscussionModel_AfterSaveDiscussion_Handler($Sender)
    {
        ...
        if ($DPModel->HasResponses($DiscussionID) && !$Session->CheckPermission('Plugins.DiscussionPolls.Manage')) {

            Gdn::Controller()->InformMessage(T('Plugins.DiscussionPolls.UnableToEditAfterResponses',
                'You do not have permission to edit a poll with responses.'));
    ...
    protected function _PollInsertion($Sender)
    {
        ...
        if ($DPModel->Exists($Discussion->DiscussionID)) {
            ...
            if (!$Session->CheckPermission('Plugins.DiscussionPolls.View')) {
                ...
                echo Wrap(T('Plugins.DiscussionPolls.NoView', 'You do not have permission to view polls.'), 'div', ['class' => 'DP_AnswerForm']);
            ...
            if ($Discussion->InsertUserID == $Session->UserID || $Session->CheckPermission('Plugins.DiscussionPolls.Manage')) {
                $Tools .= Wrap(
                    Anchor(T('Delete Poll'), '/discussion/poll/delete/' . $Poll->PollID), 'li', ['id' => 'DP_Remove']
                );
            }

            if (!$Results && C('Plugins.DiscussionPolls.EnableShowResults', true)) {
                $Tools .= Wrap(
                    Anchor(T('Show Results'), '/discussion/poll/results/' . $Poll->PollID), 'li', ['id' => 'DP_Results']
                );
            }

            echo WrapIf($Tools, 'ul', ['id' => 'DP_Tools']);
        } else {
            // Poll does not exist
            if ($Discussion->InsertUserID == $Session->UserID || $Session->CheckPermission('Plugins.DiscussionPolls.Manage')) {
                echo Wrap(
                    Wrap(
                        Anchor(T('Attach Poll'), '/vanilla/post/editdiscussion/' . $Discussion->DiscussionID), 'li'), 'ul', ['id' => 'DP_Tools']
                );
            }
        }
    }
    ...
    {
        $ThemeViewLoc = CombinePaths([
            PATH_THEMES,
            Gdn::Controller()->Theme,
            'views',
            $this->GetPluginFolder(false)
        ]);
    ...
    public function Setup()
    {
        ...
        if ($PermissionModel === null) {
            throw new \Exception('Missing PermissionModel');
        }
