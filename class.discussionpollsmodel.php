...
{
    //query cache
    private static $Cache = ['Exists' => [], 'Answered' => [], 'Responses' => [], 'Partial' => [], 'Get' => []];
    ...
    public function GetID($PollID, $DatasetType = false, $Options = [])
    {
        ...
        foreach ($DBResult as $Row) {
            if (array_key_exists($Row->QuestionID, $Data['Questions'])) {
                // Just add the option
                $Data['Questions'][$Row->QuestionID]['Options'][] = ['OptionID'   => $Row->OptionID,
                                                                     'Title'      => $Row->Option,
                                                                     'CountVotes' => $Row->CountVotes
                ];
            } else {
                // First time seeing this question
                // Add it and the first option
                $Data['Questions'][$Row->QuestionID] = [
                    'QuestionID'     => $Row->QuestionID,
                    'Title'          => $Row->Question,
                    'Options'        => [['OptionID' => $Row->OptionID, 'Title' => $Row->Option, 'CountVotes' => $Row->CountVotes]],
    ...
    public function Save($FormPostValues, $Settings = false)
    {
        ...
        try {
            ...
            $this->SQL->Insert('DiscussionPolls', [
            ...
            foreach ($FormPostValues['DP_Questions'] as $Index => $Question) {
                $this->SQL
                    ->Insert('DiscussionPollQuestions', [
                            'PollID' => $PollID,
                            'Text'   => $Question
                        ]
                    );
            ...
            foreach ($QuestionIDs as $Index => $QuestionID) {
                ...
                foreach ($QuestionOptions as $Option) {
                    $this->SQL
                        ->Insert('DiscussionPollQuestionOptions', [
     ...
     * @param array $FormPostValues
     * @param int $UserID
     * @return bool False indicates the user has already voted
     * @throws Exception
     */
        ...
        try {
            ...
            foreach ($FormPostValues['DP_AnswerQuestions'] as $Index => $QuestionID) {
                ...
                if (GetValue($MemberKey, $FormPostValues)) {
                    $this->SQL
                        ->Insert('DiscussionPollAnswerPartial', [
                                'PollID'     => $FormPostValues['PollID'],
                                'QuestionID' => $QuestionID,
                                'UserID'     => $UserID,
                                'OptionID'   => $FormPostValues[$MemberKey]
    ...
    public function PurgePartialAnswers($PollID, $UserID)
    {
        ...
        //remove from db
        return $this->SQL->Delete('DiscussionPollAnswerPartial', ['PollID' => $PollID, 'UserID' => $UserID]);
    }
    ...
    public function DeleteID($PollID, $Options = [])
    {
        try {
            $this->Database->BeginTransaction();
            $this->SQL->Delete('DiscussionPolls', ['PollID' => $PollID]);
            $this->SQL->Delete('DiscussionPollQuestions', ['PollID' => $PollID]);
            $this->SQL->Delete('DiscussionPollQuestionOptions', ['PollID' => $PollID]);
            $this->SQL->Delete('DiscussionPollAnswers', ['PollID' => $PollID]);
            $this->SQL->Delete('DiscussionPollAnswerPartial', ['PollID' => $PollID]);
            //clear cache
            self::PurgeCache();
            $this->Database->CommitTransaction();
        } catch (Exception $Ex) {
     ...
     * @return boolean
     */
    public function IsClosed($DiscussionID): bool
    {
        $this->SQL
            ->Select('p.Open')
            ->From('DiscussionPolls p')
